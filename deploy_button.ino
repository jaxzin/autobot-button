...
void setupBluefruit() {
  ...
  ble.sendCommandCheckOK("AT+GAPDEVNAME=Autobot Button");
...
void loop() {
  ...
  if (! ble.isConnected()) {
      ...
      isButtonPressed = false; // Don't want old presses to send on reconnection
  }
  ...
  switch(currentState) {
    case STATE_DISCONNECTED:
        colorFollow(strip.Color(255, 0, 0), strip.Color(255, 0, 255), STRIP_ANIMATION_DELAY);
        break;
    case STATE_CONNECTED:
        ...
        break;
    case STATE_GOOD:
        lastColor = colorFade(lastColor, strip.Color(0, 255, 0), STRIP_ANIMATION_DELAY);
        break;
    case STATE_WARN:
        lastColor = colorFade(lastColor, strip.Color(255, 128, 0), STRIP_ANIMATION_DELAY);
        break;
    case STATE_ERROR:
        lastColor = colorFade(lastColor, strip.Color(255, 0, 0), STRIP_ANIMATION_DELAY);
        break;
    case STATE_UNKNOWN:
        lastColor = colorFade(lastColor, strip.Color(255, 255, 255), STRIP_ANIMATION_DELAY);
        break;
    case STATE_PENDING:
        colorFollow(lastColor, strip.Color(128, 128, 128), STRIP_ANIMATION_DELAY);
        break;
  }
...
void colorFollow(uint32_t mainColor, uint32_t cursorColor, uint8_t wait) {
  int lim = strip.numPixels();
  for(uint16_t i=0; i<lim*2; i++) {
//    strip.setPixelColor(prev(prev(i,lim),lim), mainColor);
//    strip.setPixelColor(prev(i,lim), blend(mainColor, cursorColor));
    if(i%2 == 0 ) {
      strip.setPixelColor(prev(i/2, lim), mainColor);
      strip.setPixelColor(i/2, cursorColor);
    } else {
      strip.setPixelColor(i/2, blend(mainColor,cursorColor));
      strip.setPixelColor(next(i/2, lim), blend(mainColor,cursorColor));
    }
    strip.show();
    delay(wait/2);
  }
...
uint32_t blend(uint32_t c1, uint32_t c2) {
  return strip.Color(avg(r(c1), r(c2)), avg(g(c1), g(c2)), avg(b(c1), b(c2)));
}

uint32_t blend(uint32_t c1, uint32_t c2, float ratio) {
  return strip.Color(blendInt8(r(c1), r(c2), ratio), blendInt8(g(c1), g(c2), ratio), blendInt8(b(c1), b(c2), ratio));
}

/**
 * @param r 0.0-1.0 where 0.0 return a, 1.0 returns b, 
 */
